...
const NAV_DATA = [
  ...
  },
  {
    ...
    subLinks: [
      ...
      { name: 'Chemical & Pharmacy', href: '/industries/chemical&pharmaceuticals' },
...
export default function Navbar() {
  ...
  const [mobileActiveTab, setMobileActiveTab] = useState(null); // Fix: Removed TypeScript notation
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isMobileOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsMobileOpen(false)} className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[200] lg:hidden" />
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="fixed top-0 right-0 h-full w-[85%] max-w-[400px] bg-white z-[210] lg:hidden shadow-2xl flex flex-col">
              <div className="p-6 flex items-center justify-between border-b border-slate-100">
                <Link href="/" className="relative z-[110] flex-shrink-0">
              <Image src="/logo.svg" alt="Logo" width={100} height={32} className="w-auto h-7 md:h-8 object-contain" priority />
            </Link>
                <button onClick={() => setIsMobileOpen(false)} className="p-2 bg-slate-50 rounded-full text-slate-500"><X size={20} /></button>
              </div>
              <div className="flex-1 overflow-y-auto p-6 space-y-2">
                {NAV_DATA.map((link) => (
                  <div key={link.name} className="border-b border-slate-50 last:border-0 pb-2">
                    {link.subLinks ? (
                      <>
                        <button onClick={() => setMobileActiveTab(mobileActiveTab === link.name ? null : link.name)} className="w-full flex items-center justify-between py-3 text-slate-900 font-bold text-lg">
                          {link.name}
                          <ChevronDown size={18} className={`transition-transform duration-300 ${mobileActiveTab === link.name ? 'rotate-180 text-blue-600' : 'text-slate-300'}`} />
                        </button>
                        <AnimatePresence>
                          {mobileActiveTab === link.name && (
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden bg-slate-50 rounded-xl px-4 py-2 space-y-3">
                              {link.subLinks.map((sub) => (
                                <Link key={sub.name} href={sub.href} className="block py-1 text-slate-600 font-medium text-sm hover:text-blue-600">{sub.name}</Link>
                              ))}
                            </motion.div>
                          )}
                        </AnimatePresence>
                      ...
                      <Link href={link.href} className="block py-3 text-slate-900 font-bold text-lg">{link.name}</Link>
                    )}
                  </div>
                ))}
              </div>
              <div className="p-6 bg-slate-50 border-t border-slate-100">
                <Link href="/contacts" className="w-full bg-blue-600 text-white h-14 rounded-xl font-bold text-lg flex items-center justify-center gap-3 shadow-lg active:scale-95 transition-transform">Get Started <ArrowRight size={20} /></Link>
              </div>
            </motion.div>
          </>
        )}
